...
import static com.calclab.emiteuiplugin.client.EmiteEvents.ON_STATE_CONNECTED;
import static com.calclab.emiteuiplugin.client.EmiteEvents.ON_STATE_DISCONNECTED;
...
public class MultiChatPresenter implements MultiChat {
    ...
    private void createXmppListeners() {
        xmpp.getSession().addListener(new SessionListener() {
            public void onStateChanged(final State old, final State current) {
                ...
                case connected:
                    doAfterLogin();
                    listener.doAction(ON_STATE_CONNECTED, null);
                    break;
                case connecting:
                    doConnecting();
                    break;
                case disconnected:
                    doAfterLogout();
                    listener.doAction(ON_STATE_DISCONNECTED, null);
                    break;
                }
            }
        });
    ...
    private void fireHighLight(final String title) {
        DefaultDispatcher.getInstance().fire(ON_HIGHTLIGHTWINDOW, title);
    }

    private void fireUnHighLight(final String title) {
        DefaultDispatcher.getInstance().fire(ON_UNHIGHTLIGHTWINDOW, title);
    }
