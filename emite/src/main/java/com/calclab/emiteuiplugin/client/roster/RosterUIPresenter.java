...
public class RosterUIPresenter implements RosterUI, AbstractPresenter {
    ...
    public void showUnavailableRosterItems(final boolean show) {
        ...
        for (Iterator<XmppURI> iterator = rosterMap.keySet().iterator(); iterator.hasNext();) {
            XmppURI jid = iterator.next();
            RosterItem item = roster.findItemByJID(jid);
            ChatUserUI user = rosterMap.get(jid);
            if (item == null) {
