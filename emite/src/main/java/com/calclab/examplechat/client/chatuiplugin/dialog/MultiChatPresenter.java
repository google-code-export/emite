...
public class MultiChatPresenter implements MultiChat, GroupChatListener, PairChatListener {
    ...
    private OwnPresence ownPresence;
	...
	this.ownPresence = new OwnPresence(OwnStatus.offline);
	...
	case online:
	case onlinecustom:
	    presenceManager.setOwnPresence(ownPresence.getStatusText(), Show.available);
	    break;
	case busy:
	case busycustom:
	...
	if (ownPresence.getStatus() != OwnStatus.offline) {
	    // we go offline via external action (for instance a kune event)
	    view.setOwnPresence(ownPresence = new OwnPresence(OwnStatus.offline));
	}
    }
	...
	this.ownPresence = ownPresence;
	switch (ownPresence.getStatus()) {
	case online:
	case onlinecustom:
	case busy:
	case busycustom:
	    // FIXME: VICENTE (Presencia), necesito una forma de ver que estoy
	    // online y solo hacer de nuevo login) y solo cambiar mi presencia
	    // ¿Qué te parece xmpp.getSession().isLoggedIn()?
		...
		xmpp.getPresenceManager().setOwnPresence("pues no se", Show.dnd);
	    }
