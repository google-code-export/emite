...
public class MultiChatSamples {
    public static void show(final DefaultDispatcher dispatcher, final String currentUserJid) {
        ...
        PairChatUser pairUser = new PairChatUser("images/person-def.gif", chatId1, "Mark", "red", presenceForTest);
        dispatcher.fire(ChatDialogPlugin.CREATE_PAIR_CHAT, pairUser);
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(chatId1), XmppJID
                .parseJID(chatId1), "hello"));
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(chatId1), XmppJID
                .parseJID(currentUserJid), "hi"));

        String chatId2 = "fran@example.com";
        PairChatUser pairUser2 = new PairChatUser("images/person-def.gif", chatId2, "frank", "cyan", presenceForTest);
        dispatcher.fire(ChatDialogPlugin.CREATE_PAIR_CHAT, pairUser2);
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(chatId2), XmppJID
        ...
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(groupChatId1),
                XmppJID.parseJID(groupChatUserAlias1), "testing"));
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(groupChatId1),
                XmppJID.parseJID(groupChatUserAlias3), "testing"));
    ...
    private static void sendTestMessages(final DefaultDispatcher dispatcher, final String chatId, final String userId1,
        ...
        for (int i = 0; i < num; i++) {
            dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(chatId),
                    XmppJID.parseJID(userId1), "hello " + userId2 + " this is only a test ;)"));
            dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatInputMessageParam(XmppJID.parseJID(chatId),
                    XmppJID.parseJID(userId2), "hello " + userId1 + " this is also only a test :)"));
        }
    }
}
