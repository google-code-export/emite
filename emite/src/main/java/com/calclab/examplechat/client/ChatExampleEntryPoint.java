...
public class ChatExampleEntryPoint implements EntryPoint {
	...
	xmpp.getRoster().addListener(new RosterListener() {
		...
		for (final RosterItem item : roster) {
		    ...
		    userSelector.addItem(item.getXmppURI().toString(), item.getXmppURI().toString());
	...
	xmpp.getChat().addListener(new ChatManagerListener() {
		...
		chat.addListener(new ChatListener() {
			...
			dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new ChatMessageParam(XmppURI.parse(message
				.getFrom()), XmppURI.parse(message.getTo()), message.getBody()));
			String text = "\nIN [" + message.getFrom() + "]\n";
