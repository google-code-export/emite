...
public class ChatExampleEntryPoint implements EntryPoint {
		...
		btnSamplesExtUI = new Button("Ext UI Tests", new ClickListener() {
			public void onClick(final Widget sender) {
				if (extChatDialog != null) {
					...
					Log
							.warn("Please, login and open the ext chat window before");
				}
			}
		});
	...
	private void createExtUI() {
		...
		final AbstractChatUser currentSessionUser = new AbstractChatUser(
				userNameInput.getText(), userNameInput.getText());
		extChatDialog = ChatDialogFactory.createMultiChat(currentSessionUser,
				new I18nTranslationServiceMocked(), new MultiChatListener() {
					...
					public void onSendMessage(final GroupChat groupChat,
							final String message) {
					}

					public void onSendMessage(final PairChatUser toUserChat,
							final String message) {
					...
					public void setGroupChatSubject(final GroupChat groupChat,
	...
	private VerticalPanel createMessagePane() {
		...
		userSelector.addClickListener(new ClickListener() {
			public void onClick(final Widget arg0) {
				final String jid = userSelector.getValue(userSelector
						.getSelectedIndex());
				toIn.setText(jid);
			}
		});
