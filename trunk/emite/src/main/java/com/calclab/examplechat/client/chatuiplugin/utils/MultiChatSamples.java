...
import com.calclab.examplechat.client.chatuiplugin.GroupChatUserAddActionParam;
...
public class MultiChatSamples {
    public static void show(final DefaultDispatcher dispatcher, final String currentUserJid) {
        ...
        dispatcher.fire(ChatDialogPlugin.CREATE_GROUP_CHAT, new CreateGroupChatActionParams(groupChatId1,
                groupChatUserAlias1, GroupChatUser.MODERADOR));
        String groupChatId2 = "chat2@rooms.localhost";
        String groupChatUserAlias2 = "luther";
        dispatcher.fire(ChatDialogPlugin.CREATE_GROUP_CHAT, new CreateGroupChatActionParams(groupChatId2,
                groupChatUserAlias2, GroupChatUser.PARTICIPANT));

        dispatcher.fire(ChatDialogPlugin.SET_GROUPCHAT_SUBJECT, new GroupChatSubject(groupChatId1,
        ...
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new AbstractChatInputMessage(groupChatId1,
                groupChatUserAlias1, "testing"));
        dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new AbstractChatInputMessage(groupChatId1,
                groupChatUserAlias3, "testing"));
    ...
    private static void sendTestMessages(final DefaultDispatcher dispatcher, final String chatId, final String userId1,
        ...
        for (int i = 0; i < num; i++) {
            dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new AbstractChatInputMessage(chatId, userId1, "hello "
                    + userId2 + " this is only a test ;)"));
            dispatcher.fire(ChatDialogPlugin.MESSAGE_RECEIVED, new AbstractChatInputMessage(chatId, userId2, "hello "
