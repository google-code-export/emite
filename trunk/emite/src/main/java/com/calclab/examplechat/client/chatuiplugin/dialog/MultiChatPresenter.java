...
public class MultiChatPresenter implements MultiChat, GroupChatListener, PairChatListener {
	...
	this.currentOwnPresence = new OwnPresence(OwnStatus.offline);
	...
	final PairChatUser otherUser = rosterUI.getUserByJid(chat.getOtherURI().getJIDAsString());
	if (otherUser == null) {
	...
	switch (currentOwnPresence.getStatus()) {
	case online:
	case onlinecustom:
	    presenceManager.setOwnPresence(currentOwnPresence.getStatusText(), Show.available);
	    break;
	case busy:
	case busycustom:
	...
	case offline:
	    ...
	    // FIXME: Dani (Presence) ... logout, no? <vjrj> pues no, estaba
	    // pensando en el caso de que pensamos que estamos offline, y nos
	    // llega un evento de que estamos conectados. Lo que ha pasado es
	    // que
	    // igual
	    // desde fuera nos ha logeado (no desde el UI), por ejemplo kune
	    // hará
	    // logins desde fuera (o el antiguo botón de login del ejemplo). He
	    // cambiando el nombre a currentOwnPresence para que se entienda
	    // mejor
	    view.setOwnPresence(currentOwnPresence = new OwnPresence(OwnStatus.online));
	    break;
	}
    }
	...
	if (currentOwnPresence.getStatus() != OwnStatus.offline) {
	    // we go offline via external action (for instance a kune event)
	    view.setOwnPresence(currentOwnPresence = new OwnPresence(OwnStatus.offline));
	}
    }
	...
	this.view = view;
	reset();
	view.setOwnPresence(new OwnPresence(OwnStatus.offline));
	createXmppListeners();
	...
	this.currentOwnPresence = ownPresence;
